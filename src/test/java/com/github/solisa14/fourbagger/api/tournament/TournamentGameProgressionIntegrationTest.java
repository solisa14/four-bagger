...
class TournamentGameProgressionIntegrationTest extends AbstractIntegrationTest {
    ...
    void tournamentGameMutations_whenOrganizerIsNotPlayer_areStillAllowed() throws Exception {
        ...
        MvcResult startMatchResult = mockMvc
                .perform(post("/api/v1/tournaments/{tournamentId}/matches/{matchId}/start",
                        tournamentId, matchId)
                                .cookie(TestCookieHelper.cookie("accessToken", orgToken)))
                .andExpect(status().isOk()).andReturn();

        String gameId = objectMapper.readTree(startMatchResult.getResponse().getContentAsString())
                .get("id").asText();

        mockMvc.perform(post("/api/v1/games/{gameId}/start", gameId)
                .cookie(TestCookieHelper.cookie("accessToken", orgToken)))
                .andExpect(status().isOk()).andExpect(jsonPath("$.status").value("IN_PROGRESS"));

        mockMvc.perform(post("/api/v1/games/{gameId}/frames", gameId)
                .cookie(TestCookieHelper.cookie("accessToken", orgToken))
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(new RecordFrameRequest(1, 0, 0, 0))))
                .andExpect(status().isCreated()).andExpect(jsonPath("$.playerOneScore").value(3));
    }
        ...
        MvcResult createResult = mockMvc
                .perform(post("/api/v1/tournaments")
                        ...
                        .content(objectMapper
                                .writeValueAsString(new CreateTournamentRequest("Test", null))))
                .andExpect(status().isCreated()).andReturn();
    ...
    void recordFrame_whenBestOfThreeFirstGameCompletes_nextGameCreatedAutomatically()
        ...
        MvcResult createResult = mockMvc
                .perform(post("/api/v1/tournaments")
                        ...
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper
                                .writeValueAsString(new CreateTournamentRequest("Bo3 Test", null))))
                .andExpect(status().isCreated()).andReturn();
        ...
        tournamentService.updateRoundSettings(tournamentId,
                userRepository.findUserByUsername("bo3org" + suffix + "user").orElseThrow(), 1, 3,
        ...
        String game1Id = objectMapper.readTree(startMatchResult.getResponse().getContentAsString())
                .get("id").asText();

        // Start and complete game 1
        mockMvc.perform(post("/api/v1/games/{gameId}/start", game1Id)
                .cookie(TestCookieHelper.cookie("accessToken", orgToken)))
                .andExpect(status().isOk());

        mockMvc.perform(post("/api/v1/games/{gameId}/frames", game1Id)
                .cookie(TestCookieHelper.cookie("accessToken", orgToken))
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(new RecordFrameRequest(4, 0, 0, 0))))
                .andExpect(status().isCreated());
        ...
        MvcResult startMatch2Result = mockMvc
                .perform(post("/api/v1/tournaments/{tournamentId}/matches/{matchId}/start",
                        tournamentId, matchId)
                                .cookie(TestCookieHelper.cookie("accessToken", orgToken)))
                .andExpect(status().isOk()).andReturn();

        String game2Id = objectMapper.readTree(startMatch2Result.getResponse().getContentAsString())
                .get("id").asText();
