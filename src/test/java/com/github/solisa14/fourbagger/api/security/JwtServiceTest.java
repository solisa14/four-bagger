...
class JwtServiceTest {
    ...
    void generateToken_whenPrincipalIsUser_includesRoleAndValidates() {
        ...
        User user = TestDataFactory.user(UUID.randomUUID(), "user1", "encoded", Role.ADMIN);

        String token = jwtService.generateToken(user);

        assertThat(jwtService.extractUsername(token)).isEqualTo("user1");
        assertThat(jwtService.extractAuthorities(token)).containsExactly(new SimpleGrantedAuthority("ROLE_ADMIN"));
    ...
    void generateToken_whenPrincipalIsUserDetails_usesGrantedAuthorities() {
        ...
        assertThat(jwtService.extractAuthorities(token)).containsExactly(new SimpleGrantedAuthority("ROLE_MANAGER"));
    }
