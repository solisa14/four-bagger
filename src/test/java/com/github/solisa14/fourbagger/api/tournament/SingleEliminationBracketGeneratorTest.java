...
class SingleEliminationBracketGeneratorTest {
    ...
    void planBracket_whenFourTeams_createsWiredSingleEliminationTree() {
        ...
        assertThat(roundOne.getMatches())
                .allSatisfy(match -> assertThat(match.isBye()).isFalse());
        assertThat(roundOne.getMatches()).allSatisfy(match -> {
            assertThat(match.getWinnerNextMatch())
                    .isEqualTo(roundTwo.getMatches().getFirst());
    ...
    void planBracket_whenSevenTeams_createsEightSlotBracketWithOneByeForTopSeed() {
        ...
        for (int matchIndex = 0; matchIndex < firstRound.size(); matchIndex++) {
            assertRoute(
                    firstRound.get(matchIndex), winners.get(1).getMatches().get(matchIndex / 2), (matchIndex % 2) + 1);
        }
    ...
    void planBracket_whenSixteenTeams_createsWiredWinnersBracket() {
        ...
        for (int matchIndex = 0; matchIndex < secondRound.size(); matchIndex++) {
            assertRoute(
                    secondRound.get(matchIndex), winners.get(2).getMatches().get(matchIndex / 2), (matchIndex % 2) + 1);
        }
    ...
    void planBracket_whenSixTeams_assignsTwoByesToTopSeeds() {
        ...
        assertThat(roundOne.getMatches().stream()
                        .filter(Match::isBye)
    ...
    void planBracket_whenRoundsAlreadyConfigured_preservesRoundRules() {
        Tournament tournament = tournament();
        TournamentRound roundOne = TournamentRound.builder()
                .tournament(tournament)
                .bracketType(BracketType.WINNERS)
                .roundNumber(1)
                .bestOf(3)
                .build();
        TournamentRound roundTwo = TournamentRound.builder()
                .tournament(tournament)
                .bracketType(BracketType.WINNERS)
                .roundNumber(2)
                .bestOf(5)
    ...
    private TournamentRound round(Tournament tournament, int roundNumber) {
        return tournament.getRounds().stream()
                .filter(round -> round.getBracketType() == BracketType.WINNERS && round.getRoundNumber() == roundNumber)
                .findFirst()
                .orElseThrow();
    }
