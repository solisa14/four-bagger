...
class TournamentBracketServiceTest {
    ...
    void planBracket_whenFourTeams_createsWiredSingleEliminationTree() {
        ...
        TournamentRound roundOne = tournament.getRounds().stream()
                .filter(r -> r.getRoundNumber() == 1).findFirst().orElseThrow();
        TournamentRound roundTwo = tournament.getRounds().stream()
        ...
        assertThat(roundOne.getMatches()).allSatisfy(match -> {
    ...
    void planBracket_whenThreeTeams_marksByeAndAutoAdvancesWinner() {
        ...
        TournamentRound roundTwo = tournament.getRounds().stream()
                .filter(r -> r.getRoundNumber() == 2).findFirst().orElseThrow();
        Match byeMatch =
                roundOne.getMatches().stream().filter(Match::isBye).findFirst().orElseThrow();
    ...
    void planBracket_whenSixTeams_assignsTwoByesToTopSeeds() {
        ...
        assertThat(roundOne.getMatches().stream().filter(Match::isBye)
    ...
    void planBracket_whenFiveTeams_assignsThreeByesToTopSeeds() {
        ...
        assertThat(roundOne.getMatches().stream().filter(Match::isBye)
                .map(m -> m.getWinner().getSeed()).toList()).containsExactlyInAnyOrder(1, 2, 3);
    }

    @Test
    void planBracket_whenRoundsAlreadyConfigured_preservesRoundRules() {
        Tournament tournament = tournament();
        TournamentRound roundOne = TournamentRound.builder().tournament(tournament).roundNumber(1)
                .bestOf(3).scoringMode(ScoringMode.EXACT).build();
        TournamentRound roundTwo = TournamentRound.builder().tournament(tournament).roundNumber(2)
                .bestOf(5).scoringMode(ScoringMode.STANDARD).build();
    ...
    private Tournament tournament() {
        return Tournament.builder().id(UUID.randomUUID()).organizer(user("organizer")).title("Test")
                .status(TournamentStatus.BRACKET_READY).joinCode("ABC123").build();
    }

    private List<TournamentTeam> seededTeams(Tournament tournament, int count) {
        return java.util.stream.IntStream.rangeClosed(1, count)
                .mapToObj(seed -> TournamentTeam.builder().id(UUID.randomUUID())
                        .tournament(tournament).playerOne(user("p" + seed)).seed(seed).build())
                .toList();
    }
