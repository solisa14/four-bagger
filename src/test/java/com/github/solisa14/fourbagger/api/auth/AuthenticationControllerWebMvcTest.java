...
@TestPropertySource(properties = {"app.security.jwt.refresh-token.expiration-ms=604800000",
...
class AuthenticationControllerWebMvcTest {
    ...
    void register_whenUsernameTooShort_returnsBadRequest() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/auth/register").contentType("application/json")
                ...
                .andExpect(status().isBadRequest()).andExpect(jsonPath("$.message")
                        .value("username: Username must be between 5 and 30 characters"));
    }
    ...
    void register_whenDataIntegrityViolationBubblesUp_returnsConflict() throws Exception {
        RegisterUserRequest request = new RegisterUserRequest("validuser", "user@example.com",
                "Password1!", "Test", "User");
    ...
    void register_whenUsernameAlreadyExists_returnsConflict() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/auth/register").contentType("application/json")
                ...
                .andExpect(jsonPath("$.message").value(
                        "User with username '" + request.username() + "' may already exist."));
    }
    ...
    void register_whenEmailAlreadyExists_returnsConflict() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/auth/register").contentType("application/json")
                ...
                .andExpect(jsonPath("$.message")
                        .value("An account with this email may already exist."));
    }
