...
class RateLimitFilterIntegrationTest extends AbstractIntegrationTest {
    ...
    @Test
    void login_whenForwardedAddressChanges_usesClientAddressFromTrustedProxiesOnly() throws Exception {
        ...
        login("untrusteduser", null, "10.0.0.6", "198.51.100.3").andExpect(status().isTooManyRequests());

        clearRateLimits();

        login("trusteduser", null, "10.0.0.5", "198.51.100.10, 203.0.113.1").andExpect(status().isUnauthorized());
        login("trusteduser", null, "10.0.0.5", "198.51.100.11, 203.0.113.1").andExpect(status().isUnauthorized());
        login("trusteduser", null, "10.0.0.5", "198.51.100.12, 203.0.113.1").andExpect(status().isTooManyRequests());
    }
    ...
    void register_andJoinCodeLookup_whenRateLimitsExceeded_returnTooManyRequests() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/tournaments/join-code/ABC123")).andExpect(status().isUnauthorized());
        mockMvc.perform(get("/api/v1/tournaments/join-code/ABC123"))
                .andExpect(status().isTooManyRequests())
                .andExpect(jsonPath("$.status").value(429));
    }

    private ResultActions login(String username, String origin, String remote, String forwarded) throws Exception {
