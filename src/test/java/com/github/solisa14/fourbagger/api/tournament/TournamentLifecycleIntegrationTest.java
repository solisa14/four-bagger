...
class TournamentLifecycleIntegrationTest extends AbstractIntegrationTest {
    ...
    void fullTournamentLifecycle_createJoinBracketConfigStartAndVerify() throws Exception {
        ...
        for (String playerToken : new String[] {p1Token, p2Token, p3Token}) {
            mockMvc.perform(post("/api/v1/tournaments/join")
                    ...
                    .andExpect(status().isOk()).andExpect(jsonPath("$.id").value(tournamentId));
        }

        // 4. Generate bracket
        mockMvc.perform(post("/api/v1/tournaments/{id}/bracket", tournamentId)
                .cookie(TestCookieHelper.cookie("accessToken", orgToken)))
                .andExpect(status().isOk()).andExpect(jsonPath("$.status").value("BRACKET_READY"))
    ...
    void organizerOnlyEndpoints_whenCalledByOutsider_returnForbidden() throws Exception {
        ...
        UUID participantId = transactionTemplate.execute(status -> tournamentRepository
                .findById(tournamentId).orElseThrow().getParticipants().getFirst().getId());
        ...
        UUID matchId = matchRepository
                .findByRound_Tournament_IdOrderByRound_RoundNumberAscMatchNumberAsc(tournamentId)
                .stream().filter(match -> !match.isBye()).findFirst().orElseThrow().getId();

        mockMvc.perform(
                post("/api/v1/tournaments/{tournamentId}/matches/{matchId}/start", tournamentId,
                        matchId).cookie(TestCookieHelper.cookie("accessToken", outsiderToken)))
                .andExpect(status().isForbidden());
        ...
        MvcResult createResult = mockMvc
                .perform(post("/api/v1/tournaments")
                        ...
                        .content(objectMapper
                                .writeValueAsString(new CreateTournamentRequest("Dup Test", null))))
                .andExpect(status().isCreated()).andReturn();

        String joinCode = objectMapper.readTree(createResult.getResponse().getContentAsString())
                .get("joinCode").asText();
    ...
    void removeParticipant_removesAndAllowsRejoin() throws Exception {
        ...
        mockMvc.perform(
                delete("/api/v1/tournaments/{id}/participants/{participantId}", tournamentId,
                        UUID.randomUUID()).cookie(TestCookieHelper.cookie("accessToken", orgToken)))
                .andExpect(status().isNotFound())
