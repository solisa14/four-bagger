...
class TournamentRepositoryTest extends AbstractDataJpaTest {
    ...
    private User savedUser(String suffix) {
        return userRepository.saveAndFlush(TestDataFactory.user(null, "user" + suffix, "encoded", Role.USER));
    }

    @Test
    void findByJoinCode_whenCodeExists_returnsTournament() {
        User organizer = savedUser("a");
        tournamentRepository.saveAndFlush(TestDataFactory.tournament(organizer, "Summer Cup", "ABC123"));
    ...
    void findDetailById_fetchesParticipantUsersBeforeEntityManagerIsCleared() {
        ...
        Tournament tournament = saveTournament(organizer, "Detail Tournament", "DETAIL", TournamentStatus.REGISTRATION);
        tournament
                .getParticipants()
    ...
    void findDetailByJoinCode_fetchesParticipantUsersBeforeEntityManagerIsCleared() {
        ...
        Tournament tournament =
                saveTournament(organizer, "Join Code Tournament", "JOIN01", TournamentStatus.REGISTRATION);
        tournament
                .getParticipants()
                .add(TournamentParticipant.builder()
                        .tournament(tournament)
    ...
    void findByOrganizerIdAndStatusIn_whenActiveAndCompletedExist_returnsOnlyActiveHosted() {
        ...
        List<Tournament> result = tournamentRepository.findByOrganizer_IdAndStatusInOrderByUpdatedAtDesc(
                organizer.getId(),
    ...
    void findParticipatingActiveTournaments_whenActiveAndCompletedExist_returnsOnlyActivePlaying() {
        ...
        Tournament active = saveTournament(organizer, "Playing Registration", "PLAY01", TournamentStatus.REGISTRATION);
        Tournament completed = saveTournament(organizer, "Playing Completed", "PLAY02", TournamentStatus.COMPLETED);
        active.getParticipants()
                .add(TournamentParticipant.builder()
                        .tournament(active)
                        .user(player)
                        .build());
        ...
        List<Tournament> result = tournamentRepository.findParticipatingActiveTournaments(
                player.getId(),
                List.of(TournamentStatus.REGISTRATION, TournamentStatus.BRACKET_READY, TournamentStatus.IN_PROGRESS));
    ...
    void findByTournament_whenBracketGenerated_persistsWinnerRoutingEdges() {
        ...
        List<Match> roundOneMatches = matches.stream()
                .filter(match -> match.getRound().getRoundNumber() == 1)
                .toList();
        ...
        assertThat(roundOneMatches)
                .allSatisfy(match -> assertThat(match.getLoserNextMatch()).isNull());
        assertThat(roundOneMatches)
                .allSatisfy(
                        match -> assertThat(match.getLoserNextMatchPosition()).isNull());
    }
    ...
    void findForResponseById_whenMatchHasResetRoutes_canMapAfterEntityManagerIsCleared() {
        ...
        tournamentBracketService.planBracket(tournament, teams);
        tournamentRepository.saveAndFlush(tournament);
        Match firstFinal = tournament.getRounds().stream()
    ...
    private Tournament saveTournament(User organizer, String title, String joinCode, TournamentStatus status) {
