...
class GameFlowIntegrationTest extends AbstractIntegrationTest {
    ...
    void gameFlow_whenResultSubmitted_completesWithWinner() throws Exception {
        ...
        MvcResult p1Profile = mockMvc.perform(
                        get("/api/v1/user/me").cookie(TestCookieHelper.cookie("accessToken", p1Token)))
                .andExpect(status().isOk())
                .andReturn();
        MvcResult p2Profile = mockMvc.perform(
                        get("/api/v1/user/me").cookie(TestCookieHelper.cookie("accessToken", p2Token)))
                .andExpect(status().isOk())
                .andReturn();

        String p1Id = objectMapper
                .readTree(p1Profile.getResponse().getContentAsString())
                .get("id")
                .asText();
        String p2Id = objectMapper
                .readTree(p2Profile.getResponse().getContentAsString())
                .get("id")
                .asText();

        CreateGameRequest createRequest = new CreateGameRequest(java.util.UUID.fromString(p2Id));
        ...
        SubmitGameResultRequest resultRequest = new SubmitGameResultRequest(java.util.UUID.fromString(p1Id), 21, 15);

        mockMvc.perform(post("/api/v1/games/{gameId}/result", gameId)
                        .cookie(TestCookieHelper.cookie("accessToken", p1Token))
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        MvcResult finalState = mockMvc.perform(
                        get("/api/v1/games/{gameId}", gameId).cookie(TestCookieHelper.cookie("accessToken", p1Token)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("COMPLETED"))
                .andReturn();
    ...
    void cancelGame_whenRequested_setsStatusToCancelled() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/games/{gameId}/cancel", gameId)
                        .cookie(TestCookieHelper.cookie("accessToken", p1Token)))
                .andExpect(status().isOk())
    ...
    void listMyGames_whenUserHasGames_returnsCurrentUsersGames() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/games/me").cookie(TestCookieHelper.cookie("accessToken", p1Token)))
                .andExpect(status().isOk())
