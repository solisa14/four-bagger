...
class GameFlowIntegrationTest extends AbstractIntegrationTest {
    ...
    void gameFlow_whenPlayedToTargetScore_completesWithWinner() throws Exception {
        ...
        // Get player 2's ID
        MvcResult p2Profile = mockMvc
                .perform(get("/api/v1/user/me")
                        .cookie(TestCookieHelper.cookie("accessToken", p2Token)))
                .andExpect(status().isOk()).andReturn();

        String p2Id = objectMapper.readTree(p2Profile.getResponse().getContentAsString()).get("id")
                .asText();

        // Create game (player 1 creates)
        CreateGameRequest createRequest =
                new CreateGameRequest(java.util.UUID.fromString(p2Id), 21);
        ...
        mockMvc.perform(post("/api/v1/games/{gameId}/start", gameId)
                .cookie(TestCookieHelper.cookie("accessToken", p1Token))).andExpect(status().isOk())
        ...
        mockMvc.perform(post("/api/v1/games/{gameId}/frames", gameId)
                ...
                .andExpect(status().isCreated()).andExpect(jsonPath("$.playerOneScore").value(12));
    ...
    void cancelGame_whenRequested_setsStatusToCancelled() throws Exception {
        ...
        CreateGameRequest createRequest =
                new CreateGameRequest(java.util.UUID.fromString(p2Id), null);
        MvcResult createResult = mockMvc
                .perform(post("/api/v1/games")
                        .cookie(TestCookieHelper.cookie("accessToken", p1Token))
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void listMyGames_whenUserHasGames_returnsCurrentUsersGames() throws Exception {
        ...
        for (int i = 0; i < 2; i++) {
            mockMvc.perform(
                    post("/api/v1/games").cookie(TestCookieHelper.cookie("accessToken", p1Token))
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(objectMapper.writeValueAsString(createRequest)))
                    .andExpect(status().isCreated());
        }
        ...
        mockMvc.perform(
                get("/api/v1/games/me").cookie(TestCookieHelper.cookie("accessToken", p1Token)))
