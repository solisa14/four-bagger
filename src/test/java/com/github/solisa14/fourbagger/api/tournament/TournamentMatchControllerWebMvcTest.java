...
class TournamentMatchControllerWebMvcTest {
    ...
    void startMatch_whenValid_returnsOkWithGameResponse() throws Exception {
        ...
        Game game = Game.builder().id(UUID.randomUUID()).gameType(GameType.SINGLES)
                .playerOne(principal).playerTwo(TestDataFactory.user(UUID.randomUUID(), "opponent",
                        "opp@example.com", "encoded", Role.USER))
                .status(GameStatus.PENDING).targetScore(21).build();

        when(tournamentMatchService.startMatch(eq(tournamentId), eq(matchId), any()))
                .thenReturn(game);

        mockMvc.perform(post("/api/v1/tournaments/{tournamentId}/matches/{matchId}/start",
                tournamentId, matchId).with(user(principal))).andExpect(status().isOk())
    ...
    void startMatch_whenTournamentNotFound_returnsNotFound() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/tournaments/{tournamentId}/matches/{matchId}/start",
                tournamentId, matchId).with(user(principal))).andExpect(status().isNotFound())
    ...
    void startMatch_whenInvalidTournamentState_returnsBadRequest() throws Exception {
        ...
        when(tournamentMatchService.startMatch(eq(tournamentId), eq(matchId), any()))
                .thenThrow(new InvalidTournamentStateException(
                        "Cannot start a match unless the tournament is IN_PROGRESS"));

        mockMvc.perform(post("/api/v1/tournaments/{tournamentId}/matches/{matchId}/start",
                tournamentId, matchId).with(user(principal))).andExpect(status().isBadRequest())
    ...
    void startMatch_whenUserIsNotOrganizer_returnsForbidden() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/tournaments/{tournamentId}/matches/{matchId}/start",
                tournamentId, matchId).with(user(principal))).andExpect(status().isForbidden())
                .andExpect(jsonPath("$.message")
    ...
    void getMatch_whenFound_returnsOkWithMatchResponse() throws Exception {
        ...
        mockMvc.perform(
                get("/api/v1/tournaments/{tournamentId}/matches/{matchId}", tournamentId, matchId)
                        .with(user(principal)))
                .andExpect(status().isOk()).andExpect(jsonPath("$.id").value(matchId.toString()))
    ...
    void getMatch_whenMatchBelongsToDifferentTournament_returnsBadRequest() throws Exception {
        ...
        when(tournamentMatchService.getMatch(tournamentId, matchId)).thenThrow(
                new InvalidTournamentStateException("Match does not belong to this tournament"));
    ...
    private Tournament tournament(UUID id) {
        User organizer = TestDataFactory.user(UUID.randomUUID(), "organizer", "org@example.com",
                "encoded", Role.USER);
        return Tournament.builder().id(id).organizer(organizer).title("Tournament")
                .status(TournamentStatus.IN_PROGRESS).joinCode("ABC123").build();
    }

    private Match match(UUID matchId, Tournament tournament) {
        TournamentRound round =
                TournamentRound.builder().id(UUID.randomUUID()).tournament(tournament)
                        .roundNumber(1).bestOf(1).scoringMode(ScoringMode.STANDARD).build();
        TournamentTeam teamOne = TournamentTeam.builder().id(UUID.randomUUID())
                .tournament(tournament).playerOne(testUser("team1-a")).seed(1).build();
        TournamentTeam teamTwo = TournamentTeam.builder().id(UUID.randomUUID())
                .tournament(tournament).playerOne(testUser("team2-a")).seed(2).build();

        return Match.builder().id(matchId).round(round).teamOne(teamOne).teamTwo(teamTwo)
                .matchNumber(1).status(MatchStatus.PENDING).build();
    }

    private User testUser(String suffix) {
        return TestDataFactory.user(UUID.randomUUID(), suffix, suffix + "@example.com", "encoded",
                Role.USER);
    }
}
