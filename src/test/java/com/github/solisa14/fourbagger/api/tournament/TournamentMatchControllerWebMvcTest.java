...
class TournamentMatchControllerWebMvcTest {
    ...
    void getMatch_whenFound_returnsOkWithMatchDetail() throws Exception {
        ...
        TournamentMatchDetailResponse detail = new TournamentMatchDetailResponse(
                matchId,
                1,
                MatchStatus.PENDING,
                false,
    ...
    void submitResult_whenValid_returnsCreatedWithMatchDetail() throws Exception {
        ...
        when(tournamentMatchResultService.submitResult(eq(tournamentId), eq(matchId), eq(1), any(), any()))
                .thenReturn(detail);
    ...
    void overrideMatchResult_whenValid_returnsOkWithMatchDetail() throws Exception {
        ...
        when(tournamentMatchOverrideService.overrideMatchResult(eq(tournamentId), eq(matchId), any(), any()))
                .thenReturn(detail);

        mockMvc.perform(post("/api/v1/tournaments/{tournamentId}/matches/{matchId}/override", tournamentId, matchId)
                        .with(user(principal))
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void auditEndpoint_isNotMapped() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/tournaments/{tournamentId}/matches/{matchId}/results/audit", tournamentId, matchId)
                        .with(user(principal)))
