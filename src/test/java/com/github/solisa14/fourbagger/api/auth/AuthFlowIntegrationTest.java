...
class AuthFlowIntegrationTest extends AbstractIntegrationTest {
    ...
    void authFlow_whenRegisterLoginRefreshLogoutExecuted_returnsExpectedAuthLifecycle()
        ...
        MvcResult registerResult = mockMvc
                .perform(post("/api/v1/auth/register").contentType("application/json")
                        .content(objectMapper.writeValueAsString(registerRequest)))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.username").value(registerRequest.username()))
                .andExpect(jsonPath("$.email").value(registerRequest.email())).andReturn();
        ...
        mockMvc.perform(
                get("/api/v1/user/me").cookie(TestCookieHelper.cookie("accessToken", accessToken)))
        ...
        String newRefreshToken =
                TestCookieHelper.extractCookieValue(refreshCookies, "refreshToken");
    ...
    void refreshToken_whenTokenIsExpired_returnsUnauthorized() throws Exception {
        ...
        RefreshToken token = refreshTokenRepository
                .findByTokenHash(refreshTokenService.hashToken(refreshToken)).orElseThrow();
