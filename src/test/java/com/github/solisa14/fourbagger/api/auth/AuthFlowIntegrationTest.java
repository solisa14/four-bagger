...
class AuthFlowIntegrationTest extends AbstractIntegrationTest {
    ...
    void authFlow_whenRegisterLoginRefreshLogoutExecuted_returnsExpectedAuthLifecycle() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/user/me").cookie(TestCookieHelper.cookie("accessToken", accessToken)))
        ...
        MvcResult logoutResult = mockMvc.perform(
                        post("/api/v1/auth/logout").cookie(TestCookieHelper.cookie("refreshToken", newRefreshToken)))
                .andExpect(status().isOk())
                .andReturn();

        List<String> logoutCookies = logoutResult.getResponse().getHeaders(HttpHeaders.SET_COOKIE);
        assertThat(TestCookieHelper.hasClearedCookie(logoutCookies, "accessToken"))
                .isTrue();
        assertThat(TestCookieHelper.hasClearedCookie(logoutCookies, "refreshToken"))
                .isTrue();
    ...
    void register_whenNamesAreOmitted_createsAccountWithNullNames() throws Exception {
        ...
        String accessToken = TestCookieHelper.extractCookieValue(
    ...
    void login_whenCalledTwice_invalidatesFirstRefreshSession() throws Exception {
        ...
        String firstRefreshToken = TestCookieHelper.extractCookieValue(
        ...
        String secondRefreshToken = TestCookieHelper.extractCookieValue(
