...
class TournamentControllerWebMvcTest {
    ...
    private final TournamentMapper detailMapper = new TournamentMapper(new TournamentBracketEligibilityPolicy());
    ...
    void setUpMapperStubs() {
        ...
        when(tournamentMapper.toTournamentListResponse(any()))
                .thenAnswer(invocation -> detailMapper.toTournamentListResponse(invocation.getArgument(0)));
        when(tournamentMapper.toTournamentSummaryResponse(any()))
                .thenAnswer(invocation -> detailMapper.toTournamentSummaryResponse(invocation.getArgument(0)));
        when(tournamentMapper.toCreateCommand(any(), any()))
                .thenAnswer(invocation ->
                        detailMapper.toCreateCommand(invocation.getArgument(0), invocation.getArgument(1)));
    }
    ...
    void createTournament_whenValidRequest_returnsCreated() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/tournaments")
                        ...
                        .content(objectMapper.writeValueAsString(new CreateTournamentRequest(
                                "TestTournament", null, null, TournamentParticipationMode.SELF_JOIN))))
    ...
    void getTournament_whenOrganizerWithEligibleParticipants_returnsManagementCapabilities() throws Exception {
        ...
        for (int i = 0; i < 3; i++) {
            tournament
                    ...
                    .add(TournamentParticipant.builder()
                            ...
                            .user(TestDataFactory.user(UUID.randomUUID(), "player" + i, "encoded", Role.USER))
                            .build());
        }
        when(tournamentService.getTournamentForUser(any(), any())).thenReturn(tournament);
        stubTournamentDetailResponse(tournament, principal);
        ...
        mockMvc.perform(get("/api/v1/tournaments/{id}", id).with(user(principal)))
                ...
                .andExpect(
                        jsonPath("$.viewerCapabilities.canRemoveParticipants").value(true));
    }
    ...
    void getTournamentByJoinCode_whenFound_returnsOk() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/tournaments/join-code/ABC123").with(user(principal)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(id.toString()))
                .andExpect(jsonPath("$.joinCode").value("ABC123"))
                .andExpect(jsonPath("$.title").value("TestTournament"))
    ...
    void listMyTournaments_returnsGroupedActiveTournamentSummaries() throws Exception {
        ...
        Tournament playing = Tournament.builder()
                .id(UUID.randomUUID())
                .organizer(TestDataFactory.authenticatedUser())
                .title("Playing Cup")
                .joinCode("PLAY01")
                .participationMode(TournamentParticipationMode.SELF_JOIN)
                .status(TournamentStatus.IN_PROGRESS)
        ...
        mockMvc.perform(get("/api/v1/tournaments/me").with(user(principal)))
                .andExpect(status().isOk())
    ...
    void listCompletedTournaments_returnsCompletedTournamentSummaries() throws Exception {
        ...
        Tournament completed = Tournament.builder()
                ...
                .build();
        when(tournamentService.listCompletedTournamentsForUser(any())).thenReturn(java.util.List.of(completed));

        mockMvc.perform(get("/api/v1/tournaments/completed").with(user(principal)))
                .andExpect(status().isOk())
    ...
    void addGuestParticipant_whenValid_returnsCreated() throws Exception {
        ...
        when(tournamentMapper.toParticipantResponse(eq(guest), any()))
                .thenReturn(new TournamentParticipantResponse(participantId, null, "Pat Riley", false));
    ...
    void updateGuestParticipant_whenValid_returnsOk() throws Exception {
        ...
        TournamentParticipant guest = TournamentParticipant.builder()
                ...
                .displayName("Pat Riley")
                .build();
        when(tournamentService.updateGuestParticipant(eq(tournamentId), any(), eq(participantId), eq("Pat Riley")))
                .thenReturn(guest);
        when(tournamentMapper.toParticipantResponse(eq(guest), any()))
                .thenReturn(new TournamentParticipantResponse(participantId, null, "Pat Riley", false));

        mockMvc.perform(patch("/api/v1/tournaments/{id}/participants/{participantId}", tournamentId, participantId)
                        .with(user(principal))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new GuestParticipantRequest("Pat Riley"))))
                .andExpect(status().isOk())
