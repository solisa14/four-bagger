...
class TournamentControllerWebMvcTest {
    ...
    private Tournament bracketReadyTournament(UUID id, User organizer) {
        return Tournament.builder().id(id).organizer(organizer).title("TestTournament")
                .joinCode("ABC123").status(TournamentStatus.BRACKET_READY).build();
    }

    private Tournament inProgressTournament(UUID id, User organizer) {
        return Tournament.builder().id(id).organizer(organizer).title("TestTournament")
                .joinCode("ABC123").status(TournamentStatus.IN_PROGRESS).build();
    }
    ...
    void createTournament_whenValidRequest_returnsCreated() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/tournaments").with(user(principal))
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper
                        .writeValueAsString(new CreateTournamentRequest("TestTournament", null))))
    ...
    void createTournament_whenDoublesType_returnsCreatedWithDoubles() throws Exception {
        ...
        Tournament tournament = TestDataFactory.tournament(id, principal, "TestTournament",
                "ABC123", GameType.DOUBLES);
        when(tournamentService.createTournament(any(CreateTournamentCommand.class)))
                .thenReturn(tournament);
        ...
        mockMvc.perform(post("/api/v1/tournaments").with(user(principal))
                ...
                .andExpect(status().isCreated()).andExpect(jsonPath("$.gameType").value("DOUBLES"));
    }
    ...
    void createTournament_whenNoGameType_defaultsToSingles() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/tournaments").with(user(principal))
                ...
                .content(objectMapper
                        .writeValueAsString(new CreateTournamentRequest("TestTournament", null))))
                .andExpect(status().isCreated()).andExpect(jsonPath("$.gameType").value("SINGLES"));
    }
    ...
    void deleteTournament_whenFound_returnsNoContent() throws Exception {
        ...
        mockMvc.perform(delete("/api/v1/tournaments/{id}", id).with(user(principal)))
    ...
    void deleteTournament_whenNotFound_returnsNotFound() throws Exception {
        User principal = authenticatedUser();
        UUID id = UUID.randomUUID();
        doThrow(new TournamentNotFoundException()).when(tournamentService).deleteTournament(eq(id),
                any());
    ...
    void startTournament_whenNotBracketReady_returnsBadRequest() throws Exception {
        ...
        doThrow(new InvalidTournamentStateException(
                "Tournament can only be started when bracket is ready")).when(tournamentService)
                        .startTournament(eq(id), any());

        mockMvc.perform(post("/api/v1/tournaments/{id}/start", id).with(user(principal)))
                .andExpect(status().isBadRequest()).andExpect(jsonPath("$.message")
                        .value("Tournament can only be started when bracket is ready"));
    }
    ...
    void startTournament_whenNotFound_returnsNotFound() throws Exception {
        User principal = authenticatedUser();
        UUID id = UUID.randomUUID();
        doThrow(new TournamentNotFoundException()).when(tournamentService).startTournament(eq(id),
                any());
    ...
    void startTournament_whenUserIsNotOrganizer_returnsForbidden() throws Exception {
        User principal = authenticatedUser();
        UUID id = UUID.randomUUID();
        doThrow(new TournamentAccessDeniedException(id)).when(tournamentService)
                .startTournament(eq(id), any());

        mockMvc.perform(post("/api/v1/tournaments/{id}/start", id).with(user(principal)))
                .andExpect(status().isForbidden()).andExpect(jsonPath("$.message")
                        .value("You are not allowed to modify tournament: " + id));
    }
    ...
    void generateBracket_whenInvalidState_returnsBadRequest() throws Exception {
        ...
        doThrow(new InvalidTournamentStateException(
                "Cannot generate or reshuffle bracket unless tournament is in REGISTRATION or BRACKET_READY"))
                        .when(tournamentService).generateBracket(eq(id), any());
    ...
    void generateBracket_whenTooFewParticipants_returnsBadRequest() throws Exception {
        ...
        UUID id = UUID.randomUUID();
        doThrow(new InvalidTournamentStateException(
                "Cannot generate bracket with 2 or fewer participants")).when(tournamentService)
                        .generateBracket(eq(id), any());

        mockMvc.perform(post("/api/v1/tournaments/{id}/bracket", id).with(user(principal)))
                .andExpect(status().isBadRequest()).andExpect(jsonPath("$.message")
                        .value("Cannot generate bracket with 2 or fewer participants"));
    }
    ...
    void generateBracket_whenNotFound_returnsNotFound() throws Exception {
        User principal = authenticatedUser();
        UUID id = UUID.randomUUID();
        doThrow(new TournamentNotFoundException()).when(tournamentService).generateBracket(eq(id),
                any());
    ...
    void generateBracket_whenDoubles_returnsOkWithDoublesTournament() throws Exception {
        User principal = authenticatedUser();
        UUID id = UUID.randomUUID();
        Tournament tournament = Tournament.builder().id(id).organizer(principal)
                .title("TestTournament").joinCode("ABC123").status(TournamentStatus.BRACKET_READY)
                .gameType(GameType.DOUBLES).build();
        doNothing().when(tournamentService).generateBracket(eq(id), any());
    ...
    void joinTournament_whenValidCode_returnsOkWithTournament() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/tournaments/join").with(user(principal))
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void joinTournament_whenAlreadyJoined_returnsConflict() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/tournaments/join").with(user(principal))
                ...
                .andExpect(status().isConflict()).andExpect(jsonPath("$.message")
                        .value("User is already registered in this tournament"));
    }

    @Test
    void joinTournament_whenNotInRegistration_returnsBadRequest() throws Exception {
        User principal = authenticatedUser();
        when(tournamentService.joinTournament(any(), any())).thenThrow(
                new InvalidTournamentStateException("Tournament is not open for registration"));
    ...
    void removeParticipant_whenValid_returnsNoContent() throws Exception {
        ...
        doNothing().when(tournamentService).removeParticipant(eq(tournamentId), any(),
                eq(participantId));

        mockMvc.perform(delete("/api/v1/tournaments/{id}/participants/{participantId}",
                tournamentId, participantId).with(user(principal)))
                .andExpect(status().isNoContent());
    }
    ...
    void removeParticipant_whenNotRegistration_returnsBadRequest() throws Exception {
        ...
        UUID participantId = UUID.randomUUID();
        doThrow(new InvalidTournamentStateException(
                "Cannot remove participants after registration")).when(tournamentService)
                        .removeParticipant(eq(tournamentId), any(), eq(participantId));

        mockMvc.perform(delete("/api/v1/tournaments/{id}/participants/{participantId}",
                tournamentId, participantId).with(user(principal)))
                .andExpect(status().isBadRequest()).andExpect(jsonPath("$.message")
                        .value("Cannot remove participants after registration"));
    }
    ...
    void removeParticipant_whenParticipantNotFound_returnsNotFound() throws Exception {
        ...
        UUID tournamentId = UUID.randomUUID();
        UUID participantId = UUID.randomUUID();
        doThrow(new TournamentParticipantNotFoundException()).when(tournamentService)
                .removeParticipant(eq(tournamentId), any(), eq(participantId));

        mockMvc.perform(delete("/api/v1/tournaments/{id}/participants/{participantId}",
                tournamentId, participantId).with(user(principal))).andExpect(status().isNotFound())
    ...
    void removeParticipant_whenTournamentNotFound_returnsNotFound() throws Exception {
        ...
        UUID tournamentId = UUID.randomUUID();
        UUID participantId = UUID.randomUUID();
        doThrow(new TournamentNotFoundException()).when(tournamentService)
                .removeParticipant(eq(tournamentId), any(), eq(participantId));
    ...
    void removeParticipant_whenUserIsNotOrganizer_returnsForbidden() throws Exception {
        ...
        UUID tournamentId = UUID.randomUUID();
        UUID participantId = UUID.randomUUID();
        doThrow(new TournamentAccessDeniedException(tournamentId)).when(tournamentService)
                .removeParticipant(eq(tournamentId), any(), eq(participantId));
    ...
    void updateRoundSettings_whenValid_returnsOkWithTournament() throws Exception {
        ...
        doNothing().when(tournamentService).updateRoundSettings(eq(id), any(), eq(1), eq(3),
                eq(ScoringMode.STANDARD));
        when(tournamentService.getTournament(id)).thenReturn(bracketReadyTournament(id, principal));

        mockMvc.perform(patch("/api/v1/tournaments/{id}/rounds/{roundNumber}", id, 1)
    ...
    void updateRoundSettings_whenInvalidBestOf_returnsBadRequest() throws Exception {
        ...
        doThrow(new InvalidRoundConfigurationException("bestOf must be one of: 1, 3, 5, or 7"))
                .when(tournamentService).updateRoundSettings(eq(id), any(), eq(1), eq(2), any());
    ...
    void updateRoundSettings_whenRoundNotFound_returnsNotFound() throws Exception {
        User principal = authenticatedUser();
        UUID id = UUID.randomUUID();
        doThrow(new TournamentRoundNotFoundException()).when(tournamentService)
                .updateRoundSettings(eq(id), any(), eq(99), any(), any());

        mockMvc.perform(patch("/api/v1/tournaments/{id}/rounds/{roundNumber}", id, 99)
                .with(user(principal)).contentType(MediaType.APPLICATION_JSON)
    ...
    void updateRoundSettings_whenNoFieldsProvided_returnsBadRequest() throws Exception {
        User principal = authenticatedUser();
        UUID id = UUID.randomUUID();
        doThrow(new InvalidRoundConfigurationException(
                "At least one round setting must be provided")).when(tournamentService)
                        .updateRoundSettings(eq(id), any(), eq(1), eq(null), eq(null));

        mockMvc.perform(patch("/api/v1/tournaments/{id}/rounds/{roundNumber}", id, 1)
                .with(user(principal)).contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper
                        .writeValueAsString(new UpdateRoundSettingsRequest(null, null))))
                .andExpect(status().isBadRequest()).andExpect(
                        jsonPath("$.message").value("At least one round setting must be provided"));
    }
