...
class TournamentOrganizerManagedIntegrationTest extends AbstractIntegrationTest {
    ...
    @Test
    void organizerManagedRoster_createAddListUpdateRemoveAndRejectUnauthorizedFlows() throws Exception {
        ...
        UUID tournamentId = UUID.fromString(objectMapper
                .readTree(createResult.getResponse().getContentAsString())
                .get("id")
                .asText());

        MvcResult addResult = mockMvc.perform(post("/api/v1/tournaments/{id}/participants", tournamentId)
                        .cookie(TestCookieHelper.cookie("accessToken", organizerToken))
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        UUID guestId = UUID.fromString(objectMapper
        ...
        mockMvc.perform(get("/api/v1/tournaments/{id}", tournamentId)
                ...
                .andExpect(
                        jsonPath("$.participants[?(@.displayName == 'Alex')]").exists())
                .andExpect(jsonPath("$.participants[?(@.displayName == 'Pat Riley')]")
                        .exists());
        ...
        mockMvc.perform(patch("/api/v1/tournaments/{id}/participants/{participantId}", tournamentId, guestId)
                        .cookie(TestCookieHelper.cookie("accessToken", organizerToken))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new GuestParticipantRequest("  Patricia  "))))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(get("/api/v1/tournaments/join-code/{joinCode}", "OMJOIN")
                        .cookie(TestCookieHelper.cookie("accessToken", outsiderToken)))
                .andExpect(status().isBadRequest())
                .andExpect(
                        jsonPath("$.message").value("Organizer-managed tournaments do not support join-code lookup"));

        mockMvc.perform(delete("/api/v1/tournaments/{id}/participants/{participantId}", tournamentId, guestId)
                        .cookie(TestCookieHelper.cookie("accessToken", organizerToken)))
    ...
    void organizerManagedRoster_generateBracket_createsGuestBackedTeams() throws Exception {
        ...
        MvcResult detail = mockMvc.perform(get("/api/v1/tournaments/{id}", tournamentId)
                        .cookie(TestCookieHelper.cookie("accessToken", organizerToken)))
                .andExpect(status().isOk())
    ...
    void organizerManagedManualDoubles_setTeamsGenerateAndReshufflePreservesPairs() throws Exception {
        ...
        mockMvc.perform(patch("/api/v1/tournaments/{id}/doubles-pairing-mode", tournamentId)
                        .cookie(TestCookieHelper.cookie("accessToken", organizerToken))
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        mockMvc.perform(put("/api/v1/tournaments/{id}/manual-teams", tournamentId)
                        .cookie(TestCookieHelper.cookie("accessToken", organizerToken))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new ReplaceManualTeamsRequest(List.of(
    ...
    private Set<Set<String>> readDoublesPairs(UUID tournamentId, String organizerToken) throws Exception {
        MvcResult detail = mockMvc.perform(get("/api/v1/tournaments/{id}", tournamentId)
                        .cookie(TestCookieHelper.cookie("accessToken", organizerToken)))
                .andExpect(status().isOk())
                .andReturn();
        ...
        teams.forEach(team -> pairs.add(Set.of(
