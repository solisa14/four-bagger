...
class TournamentMatchSupport {
    ...
    Match requireMatch(UUID matchId, UUID tournamentId) {
        Match match =
                matchRepository.findForResponseById(matchId).orElseThrow(() -> new MatchNotFoundException(matchId));
    ...
    TournamentMatchDetailResponse toDetail(Match match) {
        ...
        return tournamentMapper.toMatchDetailResponse(match, results, progressionService.nextGameNumber(match));
    }
}
