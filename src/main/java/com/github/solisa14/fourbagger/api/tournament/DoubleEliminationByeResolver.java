...
class DoubleEliminationByeResolver {
  ...
  private boolean allSourcesCompleted(
      List<Match> matches, Match destination, Match excludingSource) {
    List<Match> sources = sourcesOf(matches, destination);
    return !sources.isEmpty()
        && sources.stream()
            .allMatch(
                source ->
                    !isSameMatch(source, excludingSource)
                        && source.getStatus() == MatchStatus.COMPLETED);
  }
  ...
  private void removeTeamFromSlot(TournamentTeam team, Match destination, Integer position) {
    ...
    if (position == 1
        ...
        && sameTeam(destination.getTeamOne(), team)) {
      destination.setTeamOne(null);
    } else if (position == 2
        && destination.getTeamTwo() != null
        && sameTeam(destination.getTeamTwo(), team)) {
