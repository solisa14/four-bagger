...
public class DoubleEliminationBracketGenerator implements TournamentBracketGenerator {
    ...
    private BracketMatches rebuildBracket(Tournament tournament, int bracketSize) {
        ...
        List<List<Match>> winners = rebuildMatches(
                sortedRounds(tournament, BracketType.WINNERS), roundIndex -> bracketSize / (1 << (roundIndex + 1)));
        List<List<Match>> losers = rebuildMatches(
                sortedRounds(tournament, BracketType.LOSERS),
                roundIndex -> bracketSize / (1 << ((roundIndex / 2) + 2)));
        Match championship = rebuildMatches(sortedRounds(tournament, BracketType.FINAL), roundIndex -> 1)
                .getFirst()
                .getFirst();
        Match resetFinal = rebuildMatches(sortedRounds(tournament, BracketType.GRAND_FINAL), roundIndex -> 1)
                .getFirst()
                .getFirst();
    ...
    private void removeUnusedRounds(Tournament tournament, int winnersRoundCount, int losersRoundCount) {
        tournament.getRounds().removeIf(round -> switch (round.getBracketType()) {
    ...
    private List<TournamentRound> sortedRounds(Tournament tournament, BracketType bracketType) {
    ...
    private List<List<Match>> rebuildMatches(List<TournamentRound> rounds, MatchCountByRound matchCountByRound) {
        ...
        for (int roundIndex = 0; roundIndex < winners.size(); roundIndex++) {
            ...
            for (int matchIndex = 0; matchIndex < currentRound.size(); matchIndex++) {
                ...
                if (winnersFinal) {
                    ...
                    routeWinner(match, winners.get(roundIndex + 1).get(matchIndex / 2), (matchIndex % 2) + 1);
                }
        ...
        for (int roundIndex = 0; roundIndex < losers.size(); roundIndex++) {
            ...
            for (int matchIndex = 0; matchIndex < currentRound.size(); matchIndex++) {
                ...
                if (losersFinal) {
                    ...
                    routeWinner(match, losers.get(roundIndex + 1).get(matchIndex / 2), (matchIndex % 2) + 1);
                }
            }
        }
    }
    ...
    private void seedFirstRound(List<Match> firstRoundMatches, List<TournamentTeam> seededTeams, int bracketSize) {
    ...
    private void validateSeeds(List<TournamentTeam> seededTeams, int bracketSize) {
        ...
        for (TournamentTeam team : seededTeams) {
            ...
            if (seed <= 0 || seed > bracketSize || !assignedSeeds.add(seed)) {
                throw new InvalidTournamentStateException("Invalid seed assignment while generating bracket");
            }
        }
    }

    private void configureSeededMatch(Match match, TournamentTeam seedTop, TournamentTeam seedBottom) {
    ...
    private void validateByes(List<Match> firstRoundMatches, int teamCount, int bracketSize) {
        ...
        List<Integer> byeSeeds = byeMatches.stream()
                .map(Match::getWinner)
                .map(TournamentTeam::getSeed)
                .sorted()
                .toList();
        List<Integer> expectedSeeds =
                IntStream.rangeClosed(1, expectedByeCount).boxed().toList();
    ...
    private void autoAdvanceFirstRoundByes(List<Match> firstRoundMatches) {
        firstRoundMatches.stream()
                ...
                .forEach(match ->
                        assignTeam(match.getWinner(), match.getWinnerNextMatch(), match.getWinnerNextMatchPosition()));
    }
    ...
    private record BracketMatches(
            List<List<Match>> winners, List<List<Match>> losers, Match championship, Match resetFinal) {}
}
