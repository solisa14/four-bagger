...
public class TournamentMapper {
    ...
    public CreateTournamentCommand toCreateCommand(User organizer, CreateTournamentRequest request) {
        return new CreateTournamentCommand(
                organizer, request.title(), request.gameType(), request.format(), request.participationMode());
    }
    ...
    public TournamentDetailResponse toTournamentDetailResponse(Tournament tournament, User currentViewer) {
        TournamentBracketsResponse brackets = toBracketsResponse(tournament.getRounds());
    ...
    public TournamentListResponse toTournamentListResponse(ActiveTournaments tournaments) {
        return new TournamentListResponse(
                tournaments.hosting().stream()
                        .map(this::toTournamentSummaryResponse)
                        .toList(),
                tournaments.playing().stream()
                        .map(this::toTournamentSummaryResponse)
                        .toList());
    }
    ...
    private List<TournamentParticipantResponse> toParticipantResponses(Tournament tournament, User currentViewer) {
        return tournament.getParticipants().stream()
                .sorted(Comparator.comparing(TournamentParticipant::identityLabel, String.CASE_INSENSITIVE_ORDER))
    ...
    private List<TournamentTeamResponse> toTeamResponses(Tournament tournament) {
        return tournament.getTeams().stream()
                .sorted(Comparator.comparing(TournamentTeam::getSeed, Comparator.nullsLast(Comparator.naturalOrder())))
    ...
    private TournamentBracketEligibilityResponse toBracketEligibilityResponse(BracketEligibility eligibility) {
    ...
    private TournamentViewerCapabilitiesResponse toViewerCapabilitiesResponse(
            Tournament tournament, boolean isOrganizer, boolean isParticipant, boolean bracketEligible) {
    ...
    private boolean isOrganizer(User currentViewer, Tournament tournament) {
        return currentViewer != null && tournament.getOrganizer().getId().equals(currentViewer.getId());
    }
    ...
    private boolean isParticipant(User currentViewer, Tournament tournament) {
        ...
        return tournament.getParticipants().stream().anyMatch(participant -> participant.matchesUser(currentViewerId));
    }
    ...
    private List<TournamentRoundResponse> roundsForBracket(List<TournamentRound> rounds, BracketType bracketType) {
        return rounds.stream()
    ...
    private TournamentRoundResponse toRoundResponse(TournamentRound round) {
        List<MatchResponse> matches =
                round.getMatches().stream().map(this::toMatchResponse).toList();
        return new TournamentRoundResponse(round.getBracketType(), round.getRoundNumber(), round.getBestOf(), matches);
    }
