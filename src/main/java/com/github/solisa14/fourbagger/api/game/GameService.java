...
public class GameService {
    ...
    public Game submitResult(User currentUser, UUID gameId, SubmitGameResultRequest request) {
        ...
        if (game.getStatus() != GameStatus.IN_PROGRESS) {
            throw new InvalidGameStateException(
                    "Cannot submit a result for a game that is not IN_PROGRESS. Current status: " + game.getStatus());
        }
    ...
    private void validateWinnerScore(Game game, SubmitGameResultRequest request) {
        boolean winnerIsPlayerOne = game.getPlayerOne().getId().equals(request.winnerUserId())
        ...
        boolean winnerIsPlayerTwo = game.getPlayerTwo().getId().equals(request.winnerUserId())
