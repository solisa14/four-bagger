...
class TournamentMatchController {
    ...
    ResponseEntity<TournamentMatchDetailResponse> startMatch(
            @AuthenticationPrincipal User currentUser, @PathVariable UUID tournamentId, @PathVariable UUID matchId) {
    ...
    ResponseEntity<TournamentMatchDetailResponse> getMatch(
            @AuthenticationPrincipal User currentUser, @PathVariable UUID tournamentId, @PathVariable UUID matchId) {
        return ResponseEntity.ok(tournamentMatchService.getMatchDetail(tournamentId, matchId, currentUser));
    }
    ...
    ResponseEntity<TournamentMatchDetailResponse> submitResult(
        ...
        return ResponseEntity.status(201)
                .body(tournamentMatchResultService.submitResult(
    ...
    ResponseEntity<TournamentMatchDetailResponse> overrideMatchResult(
        ...
        return ResponseEntity.ok(
                tournamentMatchOverrideService.overrideMatchResult(tournamentId, matchId, currentUser, request));
    }
}
