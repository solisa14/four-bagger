...
public class SingleEliminationBracketGenerator implements TournamentBracketGenerator {
    ...
    private void ensureRoundCount(Tournament tournament, int roundCount) {
        Map<Integer, TournamentRound> winnersRoundsByNumber = tournament.getRounds().stream()
        ...
        for (int roundNumber = 1; roundNumber <= roundCount; roundNumber++) {
            ...
            TournamentRound newRound = TournamentRound.builder()
        ...
        tournament
                .getRounds()
                .removeIf(
                        round -> round.getBracketType() == BracketType.WINNERS && round.getRoundNumber() > roundCount);
    }
    ...
    private void autoAdvanceByes(List<Match> firstRoundMatches) {
        for (Match match : firstRoundMatches) {
            ...
            } else if (match.getWinnerNextMatchPosition() != null && match.getWinnerNextMatchPosition() == 2) {
