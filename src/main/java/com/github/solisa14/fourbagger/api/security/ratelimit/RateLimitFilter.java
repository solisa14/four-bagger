...
public class RateLimitFilter extends OncePerRequestFilter {
    ...
    private Bucket getBucket(String key, RateLimitProperties.Budget budget) {
        return buckets.get(
                ...
                ignored -> Bucket.builder()
                        .addLimit(limit -> limit.capacity(budget.requests())
                                .refillGreedy(budget.requests(), Duration.ofSeconds(budget.windowSeconds())))
                        .build());
    }
    ...
    private void writeTooManyRequests(HttpServletResponse response, ConsumptionProbe probe) throws IOException {
        ...
        objectMapper.writeValue(
                ...
                new ErrorResponse(Instant.now(), HttpStatus.TOO_MANY_REQUESTS.value(), "Too many requests"));
    }
